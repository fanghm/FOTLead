...
class IssueFormViewTests(TestCase):
    def setUp(self):
        ...
        self.issue = Issue.objects.create(
            title="Test Issue", description="Test Description", type="bug"
        )
    ...
    def test_issue_form_post(self):
        ...
        response = self.client.post(
            reverse("tracker:issue_edit", args=[self.issue.pk]), data
        )
        ...
        self.assertEqual(
            response.url, reverse("tracker:issue_detail", args=[self.issue.pk])
        )
...
class IssueListViewTests(TestCase):
    def setUp(self):
        ...
        self.issue1 = Issue.objects.create(
            title="Issue 1", description="Description 1", type="bug"
        )
        ...
        self.issue3 = Issue.objects.create(
            title="Issue 3", description="Description 1", type="idea"
        )
        ...
        self.issue5 = Issue.objects.create(
            title="Issue 5", description="Description 1", type="improvement"
        )
        self.issue6 = Issue.objects.create(
            title="Issue 6", description="Description 2", type="feature"
        )
        self.issue7 = Issue.objects.create(
            title="Issue 7", description="Description 1", type="bug"
        )
    ...
    def test_issue_list_view(self):
        ...
        total_open_issues = sum(
            len(issues) for issues in response.context["grouped_open_issues"].values()
        )
        self.assertEqual(total_open_issues, 5)

        # type of issues
        self.assertEqual(len(response.context["grouped_open_issues"]), 4)

        total_closed_issues = sum(
            len(issues) for issues in response.context["grouped_closed_issues"].values()
        )
        # print(f"Total closed issues: {total_closed_issues}")
        # print(response.context['grouped_closed_issues'])

        # group of issues
        self.assertEqual(len(response.context["grouped_closed_issues"]), 3)
...
class IssueDetailViewTests(TestCase):
    def setUp(self):
        self.client = Client()
        self.user = User.objects.create_user(username="testuser", password="Test1Pwd!")
        self.issue = Issue.objects.create(
            title="Test Issue",
    ...
    def test_issue_detail_get(self):
        response = self.client.get(
            reverse("tracker:issue_detail", args=[self.issue.pk])
        )
        self.assertEqual(response.status_code, 200)
        ...
        self.assertEqual(
            response.context["comment_form"].fields["new_status"].initial,
            self.issue.status,
        )
