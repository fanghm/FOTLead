...
def jiraQuery(jql_str, field_dict):
    jira = _initJira()
    json_result = jira.search_issues(jql_str, 0, 200, json_result=True)  #, fields=list(field_dict.values())

    #print("Total results={}\n".format(len(results)))
    # print(json_result)
    with open('CB009182.json', 'w', encoding='utf-8') as f:
...
def _get_fb_info(today, full_year_sprints):
    ...
    sprints = full_year_sprints[index_of_start_fb:index_of_start_fb+4]    #tune the number to see how many fbs we need to find the day's sprint
    #print(sprints)

    i=0
    for sprint in sprints:
        i+=1
        #print(f'{today} - {sprint["fb"]} - {sprint["start_date"]} - {sprint["end_date"]}')
...
def test_get_fb_info(year):
    ...
    for i in range(1, 27):
        ...
        #print({'fb': f'FB{year[-2:]}{i:02d}', 'start': start.strftime('%Y-%m-%d'), 'end': end.strftime('%Y-%m-%d')})

        fb_dates.append({'fb': f'FB{year[-2:]}{i:02d}', 'start_date': start, 'end_date': end })
...
def update_desc():
    ...
    issue.update(assignee = { 'name': 'qwn783'}, description = '''
    Generated by bobot, please fill in numbers and don't remove this table!
    ''')

#--- main entry ----
#jql_str = '''("Feature ID" ~ CB009182-SR) and issuetype = "Competence Area" AND status not in (obsolete) order by "Item ID" '''
